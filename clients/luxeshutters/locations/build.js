 ...
 *
 * URL standard: NO trailing slash for canonical/sitemap URLs.
 * LocalBusiness schema: single global entity referenced by @id across pages.
 */
...
// Aggregate rating — kept in sync with website/src/data/clinicData.ts REVIEWS count
const AGGREGATE = { ratingValue: '5.0', reviewCount: '16', bestRating: '5', worstRating: '1' };
...
const locations = locationFiles.map(f => JSON.parse(fs.readFileSync(path.join(LOCATIONS_DIR, f), 'utf-8')));
...
for (const loc of locations) {
  ...
  const locationUrl = `${SITE_URL}/${loc.slug}`;

  // Highlights HTML (matches React design language)
  const highlightsHtml = loc.highlights.map(h =>
    `<div class="flex items-start gap-3 bg-card rounded-xl p-5 shadow-sm border border-border/40 text-left">
          <svg aria-hidden="true" class="w-5 h-5 text-primary mt-0.5 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
          <span class="text-sm">${h}</span>
        </div>`
  ).join('\n        ');

  // FAQ HTML (matches FAQSection.tsx accordion style)
  ...
  // Other locations (no trailing slash)
  ...
  // Footer location links (no trailing slash)
  ...
  // JSON-LD schema graph — single LocalBusiness entity referenced across pages via @id
...
${JSON.stringify([
    {
      ...
      aggregateRating: { '@type': 'AggregateRating', ...AGGREGATE },
    ...
    },
    {
      ...
      primaryImageOfPage: { '@type': 'ImageObject', url: `${SITE_URL}/og-image.webp` },
      breadcrumb: { '@id': `${locationUrl}#breadcrumb` },
    },
    {
      '@context': 'https://schema.org',
      '@type': 'BreadcrumbList',
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 3, name: loc.name, item: locationUrl },
      ],
    },
    {
      '@context': 'https://schema.org',
      ...
      areaServed: { '@type': 'City', name: loc.name, containedInPlace: { '@type': 'State', name: 'New South Wales' } },
      serviceType: 'Custom window furnishings and outdoor screens',
    },
    {
      '@context': 'https://schema.org',
      '@type': 'FAQPage',
...
// Generate sitemap fragment — NO trailing slash
