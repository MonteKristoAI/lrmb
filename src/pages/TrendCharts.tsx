...
import { subDays, format, isAfter, isPast } from "date-fns";
...
const TrendCharts = () => {
  ...
  const overdueTrend = Array.from({ length: 14 }, (_, i) => {
    ...
    const overdue = tasks.filter((t) => {
      if (!t.due_at || ["completed", "verified", "processed"].includes(t.status)) return false;
      const due = new Date(t.due_at);
      return due < dayEnd && isAfter(new Date(t.created_at), subDays(dayEnd, 30));
    }).length;
