...
const StaffWorkload = () => {
  ...
  return (
    <AppShell title={t("Staff Workload")}>
      <div className="p-4 space-y-4">
        {isLoading ? Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-20" />) : (
          <>
            <Card>
              <CardContent className="p-4">
                <p className="text-sm font-semibold text-foreground mb-3">{t("Active Work Orders by Staff")}</p>
                <ResponsiveContainer width="100%" height={Math.max(160, staffData.length * 40)}>
                  <BarChart data={staffData} layout="vertical">
                    ...
                    <Bar dataKey="active" fill="hsl(var(--status-in-progress))" name={t("Active")} radius={[0, 4, 4, 0]} />
                    <Bar dataKey="done" fill="hsl(var(--status-completed))" name={t("Completed")} radius={[0, 4, 4, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
              ...
              <Card key={s.id} className="cursor-pointer hover:bg-secondary/50 tap-target" onClick={() => navigate(`/admin/tasks/staff/${s.id}`)}>
                <CardContent className="p-4 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{s.active} {t("active")} · {s.done} {t("completed")}</p>
                  </div>
                  <p className="text-lg font-bold text-primary">{s.total}</p>
                </CardContent>
              </Card>
            ))}
            {staffData.length === 0 && <p className="text-muted-foreground text-center py-8">{t("No staff workload data.")}</p>}
          </>
        )}
      </div>
    </AppShell>
