...
const InspectionChecklist = () => {
  ...
  const handleToggle = async (itemId: string, checked: boolean) => {
    ...
    try {
      ...
      toast({ title: t("Failed to save response"), variant: "destructive" });
    }
  };
  ...
  const handleFlag = async (itemId: string, item: typeof items[0]) => {
    ...
    try {
      ...
      toast({ title: item.auto_create_task_on_flag ? t("Issue flagged & work order created") : t("Issue flagged") });
    } catch {
      toast({ title: t("Failed to flag issue"), variant: "destructive" });
    }
  };
  ...
  const handleComplete = async () => {
    ...
    if (answered.length < required.length) {
      toast({ title: t("Incomplete"), description: `${required.length - answered.length} ${t("required items remaining.")}`, variant: "destructive" });
      return;
    }
    try {
      await updateInspection.mutateAsync({ id, status: "completed", completed_at: new Date().toISOString() });
      toast({ title: t("Inspection completed") });
    } catch {
      toast({ title: t("Failed to complete inspection"), variant: "destructive" });
    }
  };

  if (loadingInsp) return <AppShell title={t("Inspection")}><div className="p-4 space-y-3">{Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className="h-16" />)}</div></AppShell>;
  if (!inspection) return <AppShell title={t("Inspection")}><div className="p-4 text-muted-foreground">{t("Not found.")}</div></AppShell>;

  return (
    <AppShell title={inspection.inspection_templates?.name || t("Inspection")}>
      <div className="p-4 space-y-4">
          ...
          <div className="sticky top-14 z-20 rounded-lg border border-border bg-card p-3 space-y-2">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="font-semibold text-foreground">
                {responses.filter((r) => r.response_value || r.flagged_issue).length} / {items.length} {t("addressed")}
              </span>
            </div>
                    ...
                    {!passed && !flagged && (
                      <div className="pl-7 space-y-1">
                        ...
                        <Button size="sm" variant="destructive" onClick={() => handleFlag(item.id, item)} className="tap-target text-xs gap-1">
                          <AlertTriangle className="h-3 w-3" /> {t("Flag Issue")}
                        </Button>
                      </div>
                    )}
                  </CardContent>
        ...
        {inspection.status !== "completed" && inspection.status !== "verified" && (
          <Button onClick={handleComplete} disabled={updateInspection.isPending} className="w-full tap-target gap-2 bg-status-completed hover:bg-status-completed/90 text-primary-foreground">
            <CheckCircle2 className="h-4 w-4" /> {t("Complete Inspection")}
          </Button>
        )}
      </div>
    </AppShell>
