    ...
    <AppShell title={t("Inspections")}>
      <div className="p-4 space-y-3">
        {isLoading ? Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-20" />) : (
            ...
            <Card key={ins.id} className="cursor-pointer hover:bg-secondary/50 tap-target" onClick={() => navigate(`/inspections/${ins.id}`)}>
              <CardContent className="p-4 space-y-1">
                <div className="flex items-center justify-between">
                  <p className="font-medium text-sm text-foreground">{ins.inspection_templates?.name || t("Inspection")}</p>
          ...
          )) : <p className="text-muted-foreground text-center py-8">{t("No inspections.")}</p>
        )}
      </div>
    </AppShell>
  );
