...
// v2 tokens carry `aud` (required = "ops_overview" for this fn) and `jti`
// which is checked against the revoked_share_tokens table.
type TokenPayload =
...
async function verifyShareToken(
  ...
  if (payload.v === 2) {
    ...
    // Accept ops_overview audience here. "ops_full" tokens also work.
    if (payload.aud !== "ops_overview" && payload.aud !== "ops_full") {
...
Deno.serve(async (req) => {
  ...
  try {
    const p = await verifyShareToken(token ?? "", secret, isRevoked);
