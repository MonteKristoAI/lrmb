-- L10 SPRINT-04: exception feed titles were emitting "Unit C0911" but Alex
-- asked for "Penthouse 1704" style. LRMB has 129 units with human-friendly
-- short_name populated ("Carillon 911", "Setai 3909 PH", "1HH 1040"). Prefer
-- short_name over unit_code in every exception title.
...
p1_unit_not_ready_raw AS (
  SELECT r.unit_id, u.short_name AS unit_display, r.unit_code, r.guest_name, r.arrival_time, r.arrival_date,
...
p1_unit_not_ready AS (
  SELECT DISTINCT ON (unit_id, arrival_date) 'P1'::text AS severity,
    (COALESCE(unit_display, unit_code, 'Unit ' || unit_id::text) || ' not guest ready')::text AS title,
...
p1_vip_raw AS (
  SELECT r.unit_id, u.short_name AS unit_display, r.unit_code, r.guest_name, r.arrival_date, t.id AS task_id, t.title AS task_title, t.updated_at
  FROM mv_track_reservations_latest r
  JOIN units u ON u.id::text = r.unit_id::text OR u.track_id = r.unit_id::bigint
  JOIN tasks t ON t.unit_id = u.id
...
p1_vip_open_wo AS (
  SELECT DISTINCT ON (unit_id, arrival_date) 'P1'::text AS severity,
    ('VIP arrival with open WO on ' || COALESCE(unit_display, unit_code, 'unit ' || unit_id::text))::text AS title,
...
p3_overdue_vip_raw AS (
  SELECT u.id AS unit_id, u.short_name AS unit_display, u.unit_code, t.id AS task_id, t.title AS task_title, sla_deadline(t.id) AS deadline, t.updated_at
...
p3_overdue_vip AS (
  SELECT DISTINCT ON (unit_id) 'P3'::text AS severity,
    ('Overdue on VIP-tagged unit: ' || COALESCE(unit_display, unit_code, 'unit'))::text AS title,
